...
const emit = defineEmits(['facturation-added'])
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    if (error) {
        toast.add({ title: 'Error', description: `Can't add new facturation ${error.message}`, color: 'error' })
    } else {
        toast.add({ title: 'Success', description: `New facturation ${event.data.nom} added`, color: 'success' })
        open.value = false
        emit('facturation-added')
